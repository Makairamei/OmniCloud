...
const { visibleItems: renderedFiles, handleScroll: handleListScroll, reset: resetRenderedFiles } = useIncrementalRender(sortedFiles, {
	initialCount: 80,
	step: 80,
	threshold: 240,
});
			...
			<div class="mb-2 flex flex-col items-start justify-between gap-3 sm:flex-row sm:items-center">
				<nav aria-label="Breadcrumb" class="m-0 flex flex-wrap items-center gap-1 text-2xl font-normal text-[#202124] dark:text-slate-100">
					<template v-for="(crumb, index) in breadcrumbs" :key="crumb.path">
						<button type="button" class="max-w-[220px] truncate text-left" @click="fileTreeStore.navigate(crumb.path)">
