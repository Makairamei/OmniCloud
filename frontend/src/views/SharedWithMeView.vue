...
const ownerOptions = computed(() => {
	...
	files.value.forEach((file) => {
		...
		ownerMap.set(file.email, { email: file.email, provider: file.provider || null });
	});
...
const sortedFiles = computed(() => [...filteredFiles.value].sort((a, b) => {
	const left = new Date(getModifiedTime(a) || getCreatedTime(a) || 0).getTime();
	const right = new Date(getModifiedTime(b) || getCreatedTime(b) || 0).getTime();
...
const groupedFiles = computed(() => {
	const groups = [
		{ key: 'today', label: t('recent.today'), items: [] },
		{ key: 'yesterday', label: t('recent.yesterday'), items: [] },
		{ key: 'thisWeek', label: t('recent.thisWeek'), items: [] },
		{ key: 'lastWeek', label: t('recent.lastWeek'), items: [] },
		{ key: 'thisMonth', label: t('recent.thisMonth'), items: [] },
		{ key: 'lastMonth', label: t('recent.lastMonth'), items: [] },
		{ key: 'thisYear', label: t('recent.thisYear'), items: [] },
		{ key: 'lastYear', label: t('recent.lastYear'), items: [] },
		{ key: 'older', label: t('recent.older'), items: [] },
	];

	for (const file of sortedFiles.value) {
		const key = resolveRecencyGroup(getModifiedTime(file) || getCreatedTime(file));
		groups.find((group) => group.key === key)?.items.push(file);
	}
...
const flatSelectableFiles = computed(() => groupedFiles.value.flatMap((group) => group.items));
const selectedFiles = computed(() => flatSelectableFiles.value.filter((file) => selectedFileIds.value.has(file.id)));
...
function getFileIcon(file, filled = false) {
	switch (getFileCategory(file)) {
		case 'folder': return filled ? IconFolderFilled : IconFolder;
		case 'image': return filled ? IconPhotoFilled : IconPhoto;
		case 'pdf': return filled ? IconFileFilled : IconFileTypePdf;
		case 'video': return filled ? IconVideoFilled : IconVideo;
		case 'audio': return filled ? IconFileMusicFilled : IconMusic;
		case 'archive': return filled ? IconArchiveFilled : IconFileZip;
		default: return filled ? IconFileTextFilled : IconFileText;
	}
}
...
function getTypeFilterIcon(value, filled = false) {
	switch (value) {
		...
		case 'audio': return filled ? IconFileMusicFilled : IconMusic;
		case 'archive': return filled ? IconArchiveFilled : IconFileZip;
		case 'document': return filled ? IconFileTextFilled : IconFileText;
		default: return filled ? IconFileDescriptionFilled : IconFileDescription;
	}
}
...
function matchesUpdatedFilter(value, filter) {
	...
	switch (filter) {
		case 'today': return fileDate >= todayStart;
		case 'last7': return fileDate >= last7Start;
		case 'last30': return fileDate >= last30Start;
		case 'thisYear': return fileDate >= thisYearStart;
		case 'lastYear': return fileDate >= lastYearStart && fileDate < lastYearEnd;
		default: return true;
	}
}
...
function resolveRecencyGroup(value) {
...
function selectRange(file) {
	const items = flatSelectableFiles.value;
...
function selectItem(event, file) {
	...
	closeContextMenu();
	if (event.shiftKey) return selectRange(file);
	if (event.ctrlKey || event.metaKey) return toggleSelection(file);
	replaceSelection(file);
}
...
async function openFolder(file) {
	...
	clearSelection();
	try {
		const { data } = await api.listSharedFolderChildren(file.id);
...
async function navigateToBreadcrumb(index) {
	...
	const targetFolder = nextStack.at(-1);
	if (!targetFolder) {
		await navigateToRoot();
		return;
	}
	...
	try {
		const { data } = await api.listSharedFolderChildren(targetFolder.id);
		files.value = Array.isArray(data) ? data : [];
...
async function navigateBack() {
	...
	const nextStack = folderStack.value.slice(0, -1);
	loading.value = true;
	errorMessage.value = '';
	clearSelection();
	closeContextMenu();
...
function openItemOnDoubleClick(file) {
	...
	if (canPreviewFile(file)) openPreview(file);
}
...
async function openPreview(file = contextMenu.value.file) {
	...
	previewFile.value = { ...file, previewType: getFileCategory(file), previewUrl: api.previewUrl(file.id) };
...
async function refreshShared() {
	...
	errorMessage.value = '';
	try {
		const { data } = currentFolder.value
			? await api.listSharedFolderChildren(currentFolder.value.id)
...
onMounted(() => {
	refreshShared();
	refreshTimer = window.setInterval(refreshShared, 30000);
...
<template>
	<DriveShell current-section="shared">
		<div class="relative min-h-[calc(100vh-84px)] rounded-[24px] bg-white px-4 py-[18px] pb-5 text-[#202124] dark:bg-slate-800 dark:text-slate-100 sm:px-6" @click.self="clearSelection">
				...
				<h1 class="m-0">
					<nav aria-label="Breadcrumb" class="flex flex-wrap items-center gap-1 text-2xl font-normal text-[#202124] dark:text-slate-100">
						<template v-for="(crumb, breadcrumbIndex) in breadcrumbItems" :key="`${crumb.index}:${crumb.label}`">
							<button type="button" class="max-w-[220px] truncate leading-tight" @click="navigateToBreadcrumb(crumb.index)">
								{{ crumb.label }}
							</button>
			...
			<div class="mb-3 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
				<div v-if="selectedCount" class="flex flex-wrap items-center gap-1.5 rounded-full bg-[#e8f0fe] px-2 py-1 text-[#174ea6] dark:bg-sky-500/15 dark:text-sky-200">
					...
					<button v-if="selectedCount === 1" type="button" class="inline-flex size-9 items-center justify-center rounded-full transition disabled:cursor-not-allowed disabled:opacity-45 enabled:hover:bg-[#d2e3fc] dark:enabled:hover:bg-sky-500/20" :title="t('drive.preview')" :disabled="!canPreviewSelection" @click="openPreview(primarySelectedFile)">
						...
						<div v-if="activeFilterMenu === 'owner'" class="absolute right-0 top-full z-30 mt-2 min-w-[260px] overflow-hidden rounded-2xl border border-[#e0e3e7] bg-white p-2 shadow-[0_16px_40px_rgba(32,33,36,0.16)] dark:border-slate-700 dark:bg-slate-800">
							...
							<button v-for="owner in ownerOptions" :key="owner.email" type="button" class="flex w-full items-center justify-between rounded-xl px-3 py-2.5 text-left text-sm text-[#202124] hover:bg-[#f8fafd] dark:text-slate-100 dark:hover:bg-slate-700/70" @click="applyFilter('owner', owner.email)">
								<span class="flex min-w-0 items-center gap-2">
									...
									<div v-else class="size-5 shrink-0"></div><span class="truncate">{{ owner.email }}</span>
								</span>
					...
					<div class="custom-scrollbar max-h-[min(70vh,780px)] overflow-y-auto overflow-x-hidden" @scroll="handleListScroll">
						...
						<template v-for="group in renderedGroupedFiles" :key="group.key">
							...
							<div v-for="item in group.items" :key="item.id" class="group grid min-h-[52px] cursor-default select-none grid-cols-[minmax(260px,2fr)_minmax(180px,1.1fr)_minmax(150px,1fr)_140px] items-center gap-3 border-t border-[#eceff1] px-[18px] transition dark:border-slate-700" :class="isSelected(item) ? 'bg-gradient-to-r from-[#e8f0fe] to-[#f8fbff] shadow-[inset_4px_0_0_#1a73e8] dark:from-sky-500/15 dark:to-slate-800 dark:shadow-[inset_4px_0_0_#38bdf8]' : 'hover:bg-black/[0.02] dark:hover:bg-white/6'" @click="selectItem($event, item)" @dblclick="openItemOnDoubleClick(item)" @contextmenu="openContextMenu($event, item)">
								...
								<span class="text-[#5f6368] dark:text-slate-400">{{ formatDate(getModifiedTime(item) || getCreatedTime(item)) }}</span>
					...
					<div v-for="item in group.items" :key="item.id" class="group select-none rounded-[22px] border p-4 transition hover:-translate-y-0.5 hover:border-[#d2e3fc] hover:shadow-[0_10px_30px_rgba(32,33,36,0.08)] dark:hover:border-slate-500" :class="isSelected(item) ? 'border-[#1a73e8] bg-gradient-to-br from-[#e8f0fe] to-[#f8fbff] shadow-[0_14px_34px_rgba(26,115,232,0.14)] dark:border-sky-400 dark:from-sky-500/15 dark:to-slate-800' : 'border-[#e0e3e7] bg-white dark:border-slate-700 dark:bg-slate-800'" @click="selectItem($event, item)" @dblclick="openItemOnDoubleClick(item)" @contextmenu="openContextMenu($event, item)">
						<button type="button" class="flex w-full flex-col items-start gap-4 text-left">
							...
							<div class="flex w-full items-center justify-between text-xs text-[#5f6368] dark:text-slate-400">
								<span>{{ formatDate(getModifiedTime(item) || getCreatedTime(item)) }}</span>
								<span>{{ item.is_folder ? t('common.folder') : formatBytes(item.size) }}</span>
							</div>
						</button>
					</div>
				</template>
			...
			<div v-if="contextMenu.visible" ref="contextMenuRef" class="fixed z-50 min-w-[220px] overflow-hidden rounded-2xl border border-[#e0e3e7] bg-white py-2 shadow-[0_16px_40px_rgba(32,33,36,0.2)] dark:border-slate-700 dark:bg-slate-800 dark:shadow-[0_16px_40px_rgba(15,23,42,0.45)]" :style="{ left: `${contextMenu.x}px`, top: `${contextMenu.y}px` }" @click.stop @contextmenu.stop>
				<button v-if="primarySelectedFile?.is_folder && selectedCount === 1" type="button" class="flex w-full items-center gap-3 px-4 py-3 text-left text-sm text-[#202124] hover:bg-[#f8fafd] dark:text-slate-100 dark:hover:bg-slate-700/70" @click="openSelectedItem">
					<IconFolder :size="17" :stroke="2" /><span>{{ t('common.open') }}</span>
				</button>
				<button v-if="selectedCount === 1" type="button" class="flex w-full items-center gap-3 px-4 py-3 text-left text-sm text-[#202124] hover:bg-[#f8fafd] disabled:cursor-not-allowed disabled:opacity-50 dark:text-slate-100 dark:hover:bg-slate-700/70" :disabled="!canPreviewSelection" @click="openPreview(primarySelectedFile)">
					<IconEye :size="17" :stroke="2" /><span>{{ t('drive.preview') }}</span>
				</button>
				<button v-if="canToggleStarSelection" type="button" class="flex w-full items-center gap-3 px-4 py-3 text-left text-sm text-[#202124] hover:bg-[#f8fafd] dark:text-slate-100 dark:hover:bg-slate-700/70" @click="toggleSelectedFileStar">
					<component :is="isPrimarySelectedStarred ? IconStarFilled : IconStar" :size="17" :stroke="isPrimarySelectedStarred ? 0 : 2" /><span>{{ isPrimarySelectedStarred ? t('drive.unstar') : t('drive.star') }}</span>
				</button>
				<button type="button" class="flex w-full items-center gap-3 px-4 py-3 text-left text-sm text-[#202124] hover:bg-[#f8fafd] disabled:cursor-not-allowed disabled:opacity-50 dark:text-slate-100 dark:hover:bg-slate-700/70" :disabled="!canDownloadSelection" @click="downloadSelection">
					<IconDownload :size="17" :stroke="2" /><span>{{ t('common.download') }}</span>
				</button>
				<button type="button" class="flex w-full items-center gap-3 px-4 py-3 text-left text-sm text-[#202124] hover:bg-[#f8fafd] disabled:cursor-not-allowed disabled:opacity-50 dark:text-slate-100 dark:hover:bg-slate-700/70" :disabled="!canRenameSelection" @click="renameSelectedFile">
					<IconEdit :size="17" :stroke="2" /><span>{{ t('common.edit') }}</span>
				</button>
			...
			<div v-if="isDetailsOpen && detailsFile" class="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/55 px-4" @click="closeDetails">
				<div class="w-full max-w-lg rounded-[28px] bg-white p-6 text-[#202124] shadow-[0_24px_60px_rgba(32,33,36,0.28)] dark:bg-slate-800 dark:text-slate-100" @click.stop>
					<div class="flex items-start justify-between gap-4">
						<div>
							<h3 class="text-xl font-semibold">{{ t('drive.details') }}</h3>
					...
					<dl class="mt-6 grid grid-cols-[140px_1fr] gap-x-4 gap-y-3 text-sm">
						...
						<dd>{{ detailsFile.is_folder ? '—' : formatBytes(detailsFile.size) }}</dd>
			...
			<div v-if="isPreviewOpen && previewFile" class="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/60 px-4 py-8" @click="closePreview">
				<div class="flex max-h-full w-full max-w-5xl flex-col overflow-hidden rounded-[28px] bg-white text-[#202124] shadow-[0_24px_60px_rgba(32,33,36,0.28)] dark:bg-slate-900 dark:text-slate-100" @click.stop>
					<div class="flex items-center justify-between gap-4 border-b border-[#e8eaed] px-5 py-4 dark:border-slate-800">
						...
						<div class="flex items-center gap-2"><button type="button" class="grid size-10 place-items-center rounded-full text-[#5f6368] hover:bg-black/5 dark:text-slate-400 dark:hover:bg-white/8" @click="closePreview">
								<IconX :size="18" :stroke="2" />
							</button></div>
					</div>
					<div class="relative min-h-[420px] flex-1 bg-[#f8fafd] dark:bg-slate-950">
						...
						<div v-else-if="previewFile.previewType === 'audio'" class="grid min-h-[420px] place-items-center px-6">
							<div class="w-full max-w-xl rounded-[24px] border border-[#e0e3e7] bg-white p-6 text-center dark:border-slate-700 dark:bg-slate-900">
								...
								<p class="mt-4 font-medium">{{ previewFile.file_name }}</p><audio class="mt-5 w-full" controls @loadeddata="handlePreviewLoaded" @error="handlePreviewFailed">
